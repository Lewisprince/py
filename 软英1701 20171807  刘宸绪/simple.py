...
data_train = pd.read_csv('C:/Users/DELL/Desktop/advanced python  PPTS/train.csv')  # 根据数据位置自行修改
data_train.info()
data_train.describe()
...
df['Fare'] = df['Fare'].fillna(df['Fare'].mean())   # 用平均值填充
...
from sklearn.ensemble import GradientBoostingClassifier  # 引入GradientBoostingClassifier
GBC = GradientBoostingClassifier()
GBC.fit(X_train, y_train)  # 训练数据
from sklearn.ensemble import RandomForestClassifier     # 引入RandomForestClassifier
RFC = RandomForestClassifier(n_estimators=500,random_state=6)
RFC.fit(X_train,y_train)
print('训练集准确率：\n', (GBC.score(X_train, y_train)+RFC.score(X_train,y_train))/2 ) # 分数
print('验证集准确率：\n', (GBC.score(X_val, y_val)+RFC.score(X_val,y_val))/2)

# 预测测验集
pred1 = GBC.predict(data_test)  # pred 为预测结果
pred2 = RFC.predict(data_test)
pred = (pred1+pred2)/2
pred = pred.astype(np.int)    #结果化为整数
pred = pd.DataFrame({'PassengerId': sum_id.values, 'Survived': pred})  # 格式化预测结果

pred.to_csv('pred_.csv', index=None)  # 导出数据
